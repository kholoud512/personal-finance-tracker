...
from finance_tracker.database import init_db, Transaction, Category, db
...
def test_transaction_query(test_db):
    ...
    Transaction.create(amount=30, description="Lunch", 
                      category=cat, transaction_type="expense")
    Transaction.create(amount=50, description="Dinner", 
                      category=cat, transaction_type="expense")
    
    # Query all expenses
    expenses = Transaction.select().where(
